...
An experimental Mixture-of-Experts (MoE) inference engine built to stream model weights directly from NVMe SSDs to GPU memory and bypass memory limits.
...
1. **Model Conversion:** If you are working with extremely large models that crash standard conversion utilities (like `mlx-lm`) due to memory constraints, use the provided memory-safe converter:
