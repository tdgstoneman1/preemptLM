...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `preempt/core/`               | Shared vocabulary:`enums.py` (`ParquetCompressionCodecs`), `identity.py` (`ExpertKey`, `TensorSpec` — `TensorSpec` moved here from `storage/manifest.py`), `encoding.py` (`PayloadEncoding` + `parse_payload_encoding_tag`), `protocols/` (the `IExpertBank` / `IExpertResidency` / `ExpertProvider` capability protocols). **Also currently owns event sinks** (`sinks.py`: `BaseEventSink` async ABC + `ParquetEventSink` + `JsonlFileEventSink`) — see the layering note below.                              | MLX, PyTorch, backends, model adapters, training libs |
...
| `preempt/storage/`            | The expert bank on disk:`expert_io.py` (`ExpertBank` — one `pread` per expert, `F_NOCACHE` page-cache bypass), `manifest.py` (`ExpertBankManifest`, topology + blob index), `writer.py` (`ExpertBankWriter`), `blob.py`. Implements the `IExpertBank` protocol from `core/`.                                                                                                                                                                                                                                                         | Backends, engine internals                            |
| `preempt/engine/`             | **Decisions and coordination.** `layer_resolution.py` (platform-agnostic layer resolution — `LayerCandidate`, `match_target_layers`, `resolve_target_layers`), `recorder.py` (`BaseEventRecorder`, the trace-lifecycle ABC). The streaming scheduler is now written: `DiskBackedExpertLoader` (`scheduler.py`), `ExpertCache` (`expert_cache.py`, LFRU eviction), `group_rows_by_expert` (`expert_batching.py`), chunked `generation.py`, `pipeline.py`, and `metrics.py`.                                             | `backends/*` concrete classes, model-specific code  |
| `preempt/backends/mlx_metal/` | Platform-specific execution and instrumentation:`layer_discovery.py` (enumerate MLX modules), `instrument.py` (`mlx_instrument_model` via `update_modules`/`tree_unflatten`, plus `mlx_strip_instrumented_expert_weights`), `recorder.py` (`MlxExpertRoutingRecorder`), `types.py` (`MlxWrapperFactory` alias), `instrumented/` (forked-forward capture wrappers). Streaming/residency I/O now written: `expert_kernel.py` (the expert-major kernel), `residency.py` (`MlxExpertResidency`), lazy expert load in `loader.py`. | Scheduler/policy logic                                |
