...
| `preempt/engine/`             | **Decisions and coordination.** `layer_resolution.py` (platform-agnostic layer resolution — `LayerCandidate`, `match_target_layers`, `resolve_target_layers`), `recorder.py` (`BaseEventRecorder`, the trace-lifecycle ABC). The streaming scheduler is now written: `DiskBackedExpertLoader` (`scheduler.py`), `ExpertCacheManager` (`expert_cache.py`, LFRU eviction), `group_rows_by_expert` (`expert_batching.py`), chunked `generation.py`, `pipeline.py`, and `metrics.py`.                                             | `backends/*` concrete classes, model-specific code  |
