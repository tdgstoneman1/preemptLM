...
| `preempt/storage/`            | The expert bank on disk:`expert_io.py` (`PreadExpertBank` — one `pread` per expert, `F_NOCACHE` page-cache bypass), `manifest.py` (`ExpertBankManifest`, topology + blob index), `writer.py` (`ExpertBankWriter`), `blob.py`. Implements the `IExpertBank` protocol from `core/`.                                                                                                                                                                                                                                                         | Backends, engine internals                            |
