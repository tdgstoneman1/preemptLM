...
async def test_batching_creates_one_row_group_per_full_batch(tmp_path: Path) -> None:
    ...
    async with ParquetTraceSink(path, schema=SCHEMA, batch_size=2) as sink:
        for i in range(5):
...
async def test_refuses_overwrite_by_default(tmp_path: Path) -> None:
    path = tmp_path / "out.parquet"
    path.touch()
    sink = ParquetTraceSink(path, schema=SCHEMA, batch_size=1)
    with pytest.raises(FileExistsError):
        await sink.write({"a": 1})


async def test_write_after_close_raises(tmp_path: Path) -> None:
    sink = ParquetTraceSink(tmp_path / "out.parquet", schema=SCHEMA)
    await sink.aclose()
...
def test_rejects_nonpositive_batch_size(tmp_path: Path) -> None:
    with pytest.raises(ValueError):
        ParquetTraceSink(tmp_path / "x.parquet", schema=SCHEMA, batch_size=0)
