...
**preempt** is a standalone, open-source MoE inference engine with a **trainable expert prediction head**. Its goal: run huge Mixture-of-Experts models whose weights vastly exceed available RAM by streaming expert weights from SSD, and eliminate the resulting disk bottleneck by *predicting* which experts the router will select and prefetching them ahead of demand.

Unlike llama.cpp, ds4, and colibri, preempt treats the prefetcher as a **first-class, user-trainable, swappable component**. Users can capture routing traces from their own workloads, fine-tune their own expert predictor, and hot-swap it into the engine. The trainable predictor is the primary focus of this project; the inference engine exists to evaluate and support it.

**Core Guarantee: Exact Inference.** The router's decisions are always authoritative. Predictions only affect *when* expert weights are staged into memory — never *which tokens are produced*. Wrong predictions cost wasted disk bandwidth, never correctness or quality. Never silently change model precision or router semantics; any precision compromise must be explicit and validated.

## The pitch

Sparse MoE models make running multi-trillion-parameter models on consumer hardware practical because they activate only a fraction of parameters per token, eliminating the need to hold all weights in memory at once.
...
At $h = 0.8$ — an ambitious but plausible target for learned prefetch + LRU — that's ~0.8 tok/s; enough for interactive use, and the baseline performance target for this project.
...
| `preempt/datamodel/`          | Shared data classes:`arrow.py` (attrs↔Arrow field-metadata machinery) and `tracing/` (`TraceRunContext`, `TraceStepContext`, `ExpertSelectionTrace`, `TraceMetadata`, `MoEBlockIdentifiers`). Engine-side records (`ModelManifest`, `LoadRequest`, `PredictionRequest`, …) are not written yet. `identity.py` (`ExpertKey`, `TensorSpec`)                                                                                                                                                                                                   | Backends, trainers, ORMs, concrete storage            |
...
| `preempt/engine/`             | **Decisions, inference, and coordination.** `layer_resolution.py` (platform-agnostic layer resolution — `LayerCandidate`, `match_target_layers`, `resolve_target_layers`), `recorder.py` (`BaseTraceRecorder`, the trace-lifecycle ABC). The streaming scheduler is now written: `DiskBackedExpertLoader` (`expert_io/loader.py`), `ExpertCacheManager` (`expert_io/cache_manager.py`, LFRU eviction), chunked `generation.py`, `pipeline.py`, `BaseTraceSink` async ABC + `ParquetTraceSink` + `JsonlEventSink` (`sinks.py`), and `metrics.py`. | `backends/*` concrete classes, model-specific code  |
| `preempt/backends/mlx_metal/` | Platform-specific execution and instrumentation:`instrument.py` (`mlx_instrument_model` via `update_modules`/`tree_unflatten`, plus `mlx_strip_instrumented_expert_weights`), `recorder.py` (`MlxTraceRecorder`), `types.py` (`ModuleWrapperFactory` alias), `instrumented/` (forked-forward capture wrappers). Streaming/residency I/O now written: operations in `ops/` (`experts.py`, `fused.py`, `sequential.py`), global cache interface in `expert_cache.py`, and block-local slotted caching logic via `SlottedGLU` inside `module_wrappers/qwen3_x_moe.py`.                                                                      | Scheduler/policy logic                                |
...
1. **Declare targets in TOML** — `configs/*.toml` → `TargetLayers` (`preempt/config/target_layers.py`). A target names a layer set by `layer_class`, `layer_path_glob`, and/or `block_idx`, plus an optional `count` that asserts how many layers must match. Instrumentation targets are **configuration, never hardcoded paths**; a wrong `count` triggers an explicit error at resolve time instead of silently tracing the wrong layers.
...
3. **Enumerate candidates, per backend** — `preempt/backends/mlx_metal/layer_discovery.py` walks `model.named_modules()` and parses the block index out of dotted paths. This step contains the only MLX-specific logic in target resolution.
4. **Replace modules with capture wrappers** — `instrument.py`'s `mlx_instrument_model()` swaps resolved modules via `update_modules(tree_unflatten(...))`, using an injected `ModuleWrapperFactory` (aliased in `types.py`). Wrappers live in `backends/mlx_metal/instrumented/`, one module per upstream block type (`qwen3_x_moe.py` → `Qwen3_xMoEWrapper`, built by `Qwen3_xMoEWrapper.make_wrapper_factory`).
5. **Record into a lazy buffer** — `MlxTraceRecorder` (`backends/mlx_metal/recorder.py`) implements `BaseTraceRecorder` (`engine/recorder.py`). Lifecycle is `start_trace(step_context)` → many `capture(...)` → `flush(sink)` → `stop_trace()`.
6. **Write through an async sink** — `BaseTraceSink` / `ParquetTraceSink` / `JsonlEventSink` (`engine/sinks.py`). Async context managers; all blocking I/O goes through `asyncio.to_thread`, writes serialized by an `asyncio.Lock`, batched so one row group isn't created per event.

Three conventions in this pipeline matter more than the code itself:

- **Wrappers duplicate upstream forward — verbatim router + tail, slotted grouped matmul when streaming.** `Qwen3_xMoEWrapper.__call__` copies `mlx_lm`'s `Qwen3NextSparseMoeBlock.__call__` with a `capture(...)` inserted after top-k selection — not a subclass, not a hook (MLX has no `register_forward_hook`). On the **non-streaming** path it is a line-for-line copy that delegates to `self.inner` for all weights. On the **streaming** path the router half (gate → softmax → top-k → renorm) and the tail stay verbatim; the **expert-application half is implemented with a block-local preallocated slot cache** (`SlottedGLU`). Missing experts are loaded lazily into available slots before execution, allowing the forward pass to proceed using native batched grouped matrix multiplication (`mx.gather_mm`) over the subset of cached weights rather than holding all experts resident in a stacked tensor. **Cite the upstream source URL and line in the docstring** (the existing one does) so the verbatim halves can be re-diffed when `mlx_lm` moves, and mark which parts are re-derived. The greatest risk with duplicating the upstream forward pass is silent divergence when the upstream library updates, which would alter model output and violate the EXACT INFERENCE guarantee.
- **Never `mx.eval` in the capture path.** The recorder buffers *unevaluated* `mx.array`s and defers a single batched `mx.eval` to flush time. Forcing evaluation inside the forward pass would serialize the graph and make every measurement meaningless. Conversion to Python (`.tolist()`) and unpacking the batched tensor data into per-token `ExpertSelectionTrace`s also happen at flush.
- **Arrow schema is declared on the attrs fields, not written by hand.** `preempt/datamodel/arrow.py` attaches `arrow_metadata(pa_type, serializer=…, nullable=…)` to each attrs field; `ExpertSelectionTrace.arrow_schema()` and `.as_arrow_record()` derive schema and row by walking those fields, flattening nested attrs classes (`TraceRunContext`, `TraceStepContext`, `TraceMetadata`, `MoEBlockIdentifiers`) into a flat Parquet row. **Add a field by adding an attrs field with `arrow_metadata` — never by editing a schema literal.** Bump `EXPERT_SELECTION_TRACE_SCHEMA_VERSION` when the layout changes; it is stamped into the file metadata alongside `preempt.event_type`.
...
2. **Avoid planned rewrites**
   ...
   Minimizing boilerplate and prioritizing simplicity **does not** mean cutting corners. Invest in stable boundaries early, keep implementations provisional, and minimize tight coupling so that swapping or adding a backend is a modular substitution rather than a systemic overhaul.
...
3. **Python guidelines and coding style**

- Use `typing.Protocol` for contracts; explicit dependency injection at the composition root.
- **Use the scratchpad**: Utilize the `.opencode/scratchpad/` directory for any temporary scratch work, notes, or intermediate planning.
...
- **Import ordering**
  ...
  - Place imports from `collections.abc` and `typing` first before any others (except for `__future__.annotations`).
...
- **Testing and review guidelines**
  ...
  - **Measure before optimizing**: Ensure `iobench`-style disk profiling and per-turn telemetry are implemented before introducing complex optimizations.
  - **Framework:** We use `pytest` for all testing.
  - **Current state of `tests/`:** The repository contains a comprehensive test suite split into `tests/unit/` (e.g., matching, schemas, config validation) and `tests/integration/` (e.g., cross-checking equivalence, MLX generation flows).
...
- [**warp**](https://github.com/sqliteai/waste): "Weight-Aware Runtime and Paging" — embeddable C engine running the full 2.78T Kimi K3 on a 64 GB MacBook at ~0.45–0.62 tok/s. Closest prior art to preempt's thesis: trunk resident, one aligned read per expert, bounded LRU expert cache, reads overlapped with compute, and a **lookahead router that prefetches the next layer's experts while the real router stays authoritative** — the same "changes timing, not results" invariant. Model for container layout and for measurement honesty.
...
All five of these are cloned read-only for study at **`/home/agent/repos/`** (`colibri`, `ds4`, `kimi-k3-in-c`,  `warp`). Notes before using them:
...
- Highest-value docs for current work:
  ...
  - `colibri/docs/routing-telemetry.md` — its `.coli_usage` / `ROUTE_TRACE` routing-trace format, the nearest existing analogue to `ExpertSelectionTrace`; `colibri/docs/CACHE_ROUTE.md` and `tuning.md` — cache-aware routing and its learning cache. Note `CACHE_ROUTE` **changes which experts run**; that is exactly the line preempt does not cross (EXACT INFERENCE), so read it as a contrast, not a template.
