...
| ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `preempt/core/`               | Shared vocabulary:`enums.py` (`ParquetCompressionCodecs`), `identity.py` (`ExpertKey`, `TensorSpec` — `TensorSpec` moved here from `storage/manifest.py`), `encoding.py` (`PayloadEncoding` + `parse_payload_encoding_tag`), `protocols.py` (`IExpertCache` / `IExpertLoader` / `ITokenizer` / `IModelRunner` capability protocols). **Also currently owns event sinks** (`sinks.py`: `BaseEventSink` async ABC + `ParquetEventSink` + `JsonlFileEventSink`) — see the layering note below.                              | MLX, PyTorch, backends, model adapters, training libs |
...
| `preempt/expert_bank/`            | The expert bank on disk:`banks.py` (`PreadExpertBank` — one `pread` per expert, `F_NOCACHE` page-cache bypass, `MmapExpertBank` – reads expert blobs from memory-mapped `experts.bin` ), `manifest.py` (`ExpertBankManifest`, model MoE spec + blob index), `writer.py` (`ExpertBankWriter`), `blob.py` (helper functions).                                                                                                                                                                                                                                                         | Backends, engine internals                            |
