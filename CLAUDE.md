...
- **Wrappers fork the upstream forward verbatim.** `InstrumentedQwen3NextMoE.__call__` is a line-for-line copy of `mlx_lm`'s `Qwen3NextSparseMoeBlock.__call__` with a `capture(...)` inserted after top-k selection — not a subclass, not a hook (MLX has no `register_forward_hook`). It delegates to `self.inner` for all weights. **Cite the upstream source URL and line in the docstring** (the existing one does) so the fork can be re-diffed when `mlx_lm` moves. A silently drifted fork is the highest-risk failure mode here: it changes model output, violating EXACT INFERENCE.
